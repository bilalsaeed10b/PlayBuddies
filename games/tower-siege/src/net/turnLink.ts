 ...
 *
 * Two of the other PlayWithBuddies games open a WebRTC mesh, because a volleyball
 * or a shoal of fish needs twenty position updates a second and routing those
 * through Firestore would be the largest bill on the platform. A board game
 * needs nothing of the sort: one move is one write, and a whole game is a few
 * dozen of them.
 *
 ...
 * get through. It also means a player who reloads mid-game reads the whole
 * move list straight out of the document rather than having to renegotiate.
 *
...
export class TurnLink {
  ...
  constructor(
     ...
     * four-hander has three, which is still three listeners on three small
     * documents that change once a turn.
     */
     ...
     * this the opening negotiation is destroyed by the first move that follows
   ...
   *
   * A guest does not know who moved first until the host tells it, and once it
  ...
  private async open() {
    try {
      ...
      // Clear whatever the previous game left behind before anyone can read it
      // as a live move.
   ...
   * Send a turn.
   *
   * A move made in the second before the document handle resolved is held
   * rather than dropped , losing the opening move of a game to an import that
   * had not finished is the kind of bug that reads as "multiplayer is broken".
   * Only one is ever held, which is correct here: a packet carries the whole
   * game, so the newest one makes every older one redundant.
   */
