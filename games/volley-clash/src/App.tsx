...
function OfflinePick({
  ...
  const pickedBy = useMemo(() => {
    ...
    Object.entries(picks).forEach(([id, index]) => {
      const label = `P${Number(id.split('-')[1]) + 1}`;
  ...
  return (
    <Shell title={title} coins={coins} onBack={onBack}>
      <CharacterGrid owned={owned} coins={coins} selected={null} pickedBy={pickedBy} onPick={pick} />
    </Shell>
  );
}
...
function RoomScreen({
  ...
  const pickedBy = useMemo(() => {
    ...
    for (const p of people) {
      if (p.uid !== uid && p.character !== undefined && p.character !== null) {
        (map[p.character] ??= []).push(p.displayName);
      }
    }
    return map;
  }, [people, uid]);
    ...
    <div className="mx-auto flex h-full w-full max-w-6xl flex-col overflow-y-auto overscroll-contain gap-3 p-3 sm:gap-4 sm:p-6">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 grid-rows-[auto_minmax(0,1fr)] gap-3 sm:gap-4 lg:grid-cols-3 lg:grid-rows-[minmax(0,1fr)]">
        <div className="panel order-2 min-h-0 overflow-y-auto overscroll-contain rounded-[2rem] p-3 sm:p-6 lg:order-1 lg:col-span-2">
          <CharacterGrid owned={owned} coins={coins} selected={mine ?? null} pickedBy={pickedBy} onPick={onPick} />
        </div>
