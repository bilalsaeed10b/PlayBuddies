...
import { Users, X, UserPlus, Check, MessageCircle, ChevronRight, Search } from "lucide-react";
...
export default function FriendsSidebar() {
  ...
  // Listen to RTDB status for accepted friends
  useEffect(() => {
    const unsubs: (() => void)[] = [];
    friends.forEach(f => {
      ...
      const unsub = onValue(statusRef, (snap) => {
        const val = snap.val();
        setFriendStatus(prev => ({ ...prev, [f.uid]: val?.state || "offline" }));
      });
      unsubs.push(() => unsub());
    });
  ...
  const onlineFriends = friends.filter(f => friendStatus[f.uid] === "online").length;

  useEffect(() => {
    if (!user) return;
    const { stats, setStats } = useAuthStore.getState();
    if (stats && stats.friendsOnline !== onlineFriends) {
      setStats({ ...stats, friendsOnline: onlineFriends });
    }
  }, [onlineFriends, user]);
  ...
  const inviteFriend = async (friendId: string) => {
    ...
    try {
      ...
      if (!lobbyToInvite) {
        alert("You must be in a Lobby to invite a friend!");
        return;
      }
  ...
  return (
    <>
      ...
      {!isOpen && (
        ...
        >
          <Users size={24} />
          <span className="font-bold hidden sm:inline-block">Friends ({onlineFriends})</span>
          {requests.length > 0 && (
          ...
          <>
            ...
            >
              ...
              <div className="flex px-4 pt-4 gap-2">
                {[
                  { id: "friends", label: `Online (${onlineFriends})` },
                        ...
                        <div key={f.uid} className="glass p-3 rounded-2xl border border-white/5 flex items-center justify-between group">
                          <div className="flex items-center gap-3">
                            <div className="relative">
                              ...
                              <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-[#121212] ${friendStatus[f.uid] === 'online' ? 'bg-green-500 shadow-[0_0_8px_#22c55e]' : 'bg-gray-500'}`} />
                            </div>
                            <div>
                              <p className="text-sm font-bold text-white">{f.displayName}</p>
                              <p className="text-xs text-text-muted">{friendStatus[f.uid] === 'online' ? 'Online' : 'Offline'}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-1">
                            <button
                              onClick={() => inviteFriend(f.uid)}
                              className="text-primary p-2 hover:bg-primary/20 rounded-xl transition-colors"
                ...
                {tab === "add" && (
                  <div className="space-y-6">
                    ...
                    <div className="glass p-4 rounded-2xl flex flex-col items-center justify-center text-center gap-2 border border-primary/30 bg-primary/5">
                       <p className="text-sm text-primary font-bold">Your Web Code</p>
                       <div 
                         onClick={() => {
                       ...
                       <p className="text-xs text-text-muted">Give this code to a friend!</p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {searchResults.length === 0 && searchQuery.trim() !== "" && !isSearching && (
                         <div className="text-center text-text-muted text-sm mt-4">
                            No players found with code {searchQuery}.
                         </div>
                      )}
                    </div>
                  </div>
