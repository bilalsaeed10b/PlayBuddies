...
export default function Game({ 
      ...
      if (user) {
        ...
        try {
          ...
          if (error.code === 'auth/admin-restricted-operation') {
            ...
            console.log("[Multiplayer] Anonymous auth is disabled. User must sign in with Google.");
            showToast("Please sign in with Google to play multiplayer", "info");
          } else {
      ...
      if (!roomSnap.exists()) {
        if (isHost) {
          ...
          await setDoc(roomRef, {
            ...
            players: {
              [uid]: { id: uid, role: null, ready: false }
            },
            chat: [],
            collectedGems: {}
          });
        ...
        await updateDoc(roomRef, {
          [`players.${uid}`]: { id: uid, role: null, ready: false }
        });
      }
    };
    ...
    const loop = (time: number) => {
      ...
      if (currentEngine.level.worldSettings?.chaosMode) {
        if (Math.random() > 0.99) {
          ...
          (currentEngine as any).gravity = (Math.random() * 0.4 + 0.1) * (currentEngine.level.worldSettings.gravityMultiplier || 1);
        }
        if (Math.random() > 0.98) {
          // Random screen shake
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-4 font-mono">
      ...
      <div className="relative w-full max-w-[800px] aspect-[4/3] bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-800 shadow-2xl">
        <canvas 
          ref={canvasRef} 
          width={CANVAS_WIDTH} 
      ...
      <div className="mt-8 flex gap-8 text-zinc-500 text-xs uppercase tracking-widest">
        <div className="flex items-center gap-2">
          <Monitor size={14} />
